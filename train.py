...
def main(opt):
    ...
    if "yolov5" in yolo_model_name.lower():
        ...
        try:
            ...
            print(ex)
    elif "yolo11n" in yolo_model_name.lower():
        try:
            model = yolov8(yolo_model_name)            
            params = dict()
            with open(hyp_file) as file:
    ...
    elif "sideload" in yolo_model_name.lower():

        side_yolo = yolov8("yolo_config/yolo11n-sideCNN_yolobackbone_SE.yaml").load("sideload_SE_gating.pt")
...
def parse_opt(known=False):
    ...
    parser.add_argument('--amp',
                        help='implement Automatic Mixed Precision (AMP) training')
    parser.add_argument('--device',
                        help='gpu deice')
    parser.add_argument('--freeze',
                        type=lambda x: int(x) if x.isdigit() else list(map(int, x.split(','))),
                        help='Freeze backbone layers. Accepts an integer or a comma-separated list of integers.',
                        default=0)
